...
export const ContextProvider = ({children}) => {
  ...
  const handleInfoOpenClick = () => {setInfoOpen(!infoOpen)};

  const [specialRulesOpen, setSpecialRulesOpen] = useState(false)
  const handleSpecialRulesOpenClick = () => {setSpecialRulesOpen(!specialRulesOpen)}
  ...
  const handleEraseDefaultDamage = () => {setDamage('')};
  ...
  const handleEraseDefaultCriticalHitsOn = () => {setCriticalHitsOn('')}
  ...
  const handleEraseDefaultCriticalWoundsOn = () => {setCriticalWoundsOn('')}
  ...
  const handleDevastatingWoundsClick = () => {setDevastatingWounds(!devastatingWounds)};

  const [lance, setLance] = useState(false);
  const handleLanceClick = () => {setLance(!lance)};

  const [torrent, setTorrent] = useState(false);
  const handleTorrentClick = () => {setTorrent(!torrent)}

  const [twinLinked, setTwinLinked] = useState(false);
  const handleTwinLinkedClick = () => {setTwinLinked(!twinLinked)}
  ...
  const numberNeeded = (ordinarySuccessesTreshold, critsOn) => {
    ...
    return ordinarySuccessesTreshold < critsOn ? ordinarySuccessesTreshold : critsOn
  }
  ...
  const convertToNumber = (string) => {
    ...
    let erraticAttack = {numberOfDice: '', diceInUseAvgResult: '', fixedBonus: ''};
  
    //input validation
    if (input[input.indexOf('D') + 1] !== '3' && input[input.indexOf('D') + 1] !== '6') {
      return {error: errorMessage};
    }
  
    if (input[input.indexOf('D') + 1] === '3') erraticAttack.diceInUseAvgResult = 2;
    if (input[input.indexOf('D') + 1] === '6') erraticAttack.diceInUseAvgResult = 3.5;
  
    //input validation
    if (
      parseInt(input[input.indexOf('D') - 1]) * 1 != input[input.indexOf('D') - 1]
    ...
    if (parseInt(input[input.indexOf('D') - 1]) * 1 == input[input.indexOf('D') - 1]) {
  ...
  const hitRoll = (numberOfDice, numberNeeded) => {
    ...
    if (sustainedHits) {
        output = output + (sortedSuccesses(output, numberNeeded, criticalHitsOn).criticalOnes) * sustainedHits
    }
  ...
  const woundRoll = (numberOfDice, numberNeeded) => {
    ...
    if (devastatingWounds) {
      if (anti && anti <= criticalWoundsOn) {
        ...
        unsavableWounds = sortedSuccesses(output, woundsOn, criticalWoundsOn).criticalOnes
        output = sortedSuccesses(output, woundsOn, criticalWoundsOn).ordinaryOnes;
      }
    }
    ...
    if (twinLinked) {
      ...
      if (devastatingWounds) {
        if (anti && anti <= criticalWoundsOn) {
          unsavableWounds = sortedSuccesses(reRollOutput, woundsOn, anti).criticalOnes
          reRollOutput = sortedSuccesses(reRollOutput, woundsOn, anti).ordinaryOnes;
        } else {
          unsavableWounds = sortedSuccesses(reRollOutput, woundsOn, criticalWoundsOn).criticalOnes
          reRollOutput = sortedSuccesses(reRollOutput, woundsOn, criticalWoundsOn).ordinaryOnes;
        }
      }
  ...
  const getAttackResult = () => {
    const inputsToCheckValues = [convertToNumber(attacks), hitsOn, woundsOn, savesOn, convertToNumber(damage), criticalHitsOn, criticalWoundsOn, anti, feelNoPain, sustainedHits]
    const inputsToCheckKeys = ['attacks', 'hits on', 'wounds on', 'saves on', 'damage', 'crit. hits on', 'crit wounds on', 'anti', 'feel no pain', 'sustained hits']
    let errors = [];
    let errorAlert = ''
    ...
    for (let i = 0; i < inputsToCheckValues.length; i++) {
      ...
      if (i > 0 && i < 9 && i !== 4) {
        if (
          inputsToCheckValues[i] 
          && (Number(inputsToCheckValues[i]) < 2 || Number(inputsToCheckValues[i]) > 6)
        ) {
          errors.push(inputsToCheckKeys[i])
        }
      }
  ...
  /* reset btn - reset */
  const handleReset = () => {
    const defaultEmptyStringStates = [setAttacks, setHitsOn, setWoundsOn, setSavesOn, setAnti, setFeelNoPain, setSustainedHits, setAttackResult];
    for (let state of defaultEmptyStringStates) state('')

    const defultFalseStates = [setDevastatingWounds, setLance, setTorrent, setTwinLinked];
  ...
  const hadleAttackResultChange = () => {setAttackResult(getAttackResult())}
